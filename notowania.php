...
else $wartosc = $rekordzista['numer_not'];
$ostatni = $rekordzista[2];

$current_year = date('Y', strtotime(''.$rekordzista[1].''));

$sql10 = "SELECT * from notowania where numer_not=$wartosc limit 1"; // 2
$query10 = $polaczenie->query($sql10);
$used_notowanie = mysqli_fetch_array($query10);

$used_year = date('Y', strtotime(''.$used_notowanie[1].''));
...
if ($ostatni>3) {
  ...
  echo '<div class="slider">
      ...
      while ($lp<=$current_year) {
        // PIERWSZE NOTOWANIE DANEGO ROKU
        $sql10 = "SELECT * from notowania where year(data)=$lp order by numer_not limit 1"; // 2
        $query10 = $polaczenie->query($sql10);
        $first_not = mysqli_fetch_array($query10);
        $name_of_first = $first_not[2];
        if ($lp == $used_year) {
          echo '<li class= "current-not"><h3><a href="notowania.php?id='.$name_of_first.'">'.$lp.'</a></h3></li>';
        }
        else {
          echo '<li><h3><a href="notowania.php?id='.$name_of_first.'">'.$lp.'</a></h3></li>';
        }
  ...
  echo '<div class="slider">
      ...
      while ($lp<=$ostatni) {
        if ($lp == $wartosc) {
          echo '<li class= "current-not"><h3><a href="notowania.php?id='.$lp.'">'.$lp.'</a></h3></li>';
        }
        else {
          echo '<li><h3><a href="notowania.php?id='.$lp.'">'.$lp.'</a></h3></li>';
        }
